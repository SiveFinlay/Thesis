...
% NC: Might be a good idea to link with intro. Eg In the introduction, I explained why it is important to study patterns of morphological diversity in tenrecs. In this chapter...
...
% NC: I think for the thesis it's a good idea to really carefully lay out your hypotheses. So expand on the last two sentences. Make a new paragraph for them and just expand what you've written - so explai nthat tenrecs do this this and this, whereas golden moles only live underground. And hopefully add a reference to a different ecologically diverse group and non ecological diverse group that support your hypothesis. Don't call them hypotheses though - just predictions.
	
	%I originally included the following paragraphs but then I thought that it sounded too much like an abstract
	% NC: Yes good idea, leave this out
...
%----------------------------------------------------	
	...
	In chapter \ref{chap:methods} (figure\ref{fig:flow}), I described how I photographed skulls and summarised their morphologies using landmark morphometrics. %In total, I photographed 99 different species from seven different mammal Families (table \ref{tab:species.measured}). However, here I am only using part of this full data set to compare morphological diversity of tenrecs to their closest relatives, the golden moles.
	% NC: You already say in the methods chapter that you did this many. Here just stick to what you used.
	I used photographs of 182 skulls in dorsal view (148 tenrecs and 34 golden moles), 173 skulls in ventral view (141 tenrecs and 32 golden moles), 171 skulls in lateral view (140 tenrecs and 31 golden moles) and 181 mandibles in lateral view (147 tenrecs and 34 golden moles). These samples represent 31 species of tenrec (out of the total 34 in the Family \citep{Olson2013}) and 12 species of golden moles (out of a total of 21 species in the Family, \citep{Asher2010}). Note that the number of skulls I used varies slightly for each analysis because some skulls were damaged in certain views.
	% NC: Start with what you did, then the caveat of why it's not perfect
	...
	% NC: OK - just check when you read the full thing through that it's not too repetitive
	...
	% NC: I think this should be with the previous paragraph
...
% NC: Be a bit careful with paragraphs, you seem to make them a bit randomly. If you're following the same idea stay in the same paragraph. Don't just use them to split stuff up.
	...
	% NC: First and Second, or Firstly, and Secondly
...
%--------------------------------------
% NC: I added a bit here as the previous sentence seemed a bit abrupt.
...
% NC: This looks good so maybe leave out the warps for now
...
%----------------------------------------------
	...
	% NC: Be consistent. Either no spaces between = and numbers (i.e n=31 which is what you've done up to now) or spaces round them all (i.e. n = 31, p = 0.001). Same for < and >. And no gap (ever even if you want to put spaces round =) between F and its degrees of freedom. Also an issue with mean+-se in table, no space on both sides or space on both sides.
