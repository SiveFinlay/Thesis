...
%----------------------------------------------------	
	...
	% SF: I combined two paragraphs to make the one above
...
% NC: Be a bit careful with paragraphs, you seem to make them a bit randomly. If you're following the same idea stay in the same paragraph. Don't just use them to split stuff up.
	...
	%********SF: COME BACK TO HERE and put in an equation************
	%ASK THOMAS ABOUT WRITING EQUATIONS IN LATEK
...
%----------------------------------------------
	...
	Firstly, I compared the position of each Family within the morphospace plots. Tenrecs and golden moles occupy significantly different positions in the dorsal (npMANOVA, F \textsubscript{1,42}=68.13, R$^2$=0.62, p=0.001 ), ventral (npMANOVA, F\textsubscript{1,42}=103.33, R$^2$=0.72 , p=0.001 ) and lateral (npMANOVA, F\textsubscript{1,42} = 76.7, R$^2$ =0.652, p=0.001 ) skull morphospaces as well as in the mandible morphospace (npMANOVA, F \textsubscript{1,42}=60.38, R$^2$=0.59, p=0.001),  indicating that the Families have very different, non-overlapping cranial and mandible morphologies (figure \ref{fig:FourPCA}). 
	...
	%SF: I changed the text to have no spacing around = and the tables have spaces around the +/- and </> because otherwise it looks too cluttered
...
%Results table: I changed the format to make it fit better
	...
	%SF: I'm confused about the df: when I run the t test on the skulls dorsal, ventral, lateral and on the mandibles, I get different df each time (29.88, 26.99, 35.03 and 25.85 respectively). All of the analyses are comparing groups of equal sample sizes (31 tenrecs vs. 12 golden moles) so why do the df change?
	
	%********SF COME BACK TO HERE and sort out the df issue*****
%------------------------------------
	...
	%Moved this paragraph up from the discussion
...
%----------------------------------------------------
%SF: I moved the discussion to a separate chapter
