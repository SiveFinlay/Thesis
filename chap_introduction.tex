...
% NC: I reckon this is all one paragraph - need quantification + it's hard to do
...
%Link to tenrecs
	...
	% NC: I think you can drop the subheadings as this section is quite short, and maybe just have one subheading for structure and contents of the thesis if you think you need it. So this sentence above can be the start of the next paragraph on tenrecs... 
%----------------------------------------------------------
...
%Example of both an adaptive radiation and convergence
	Within this vast size range there are tenrecs which convergently resemble shrews (\textit{Microgale} tenrecs), moles (\textit{Oryzorictes} tenrecs) and hedgehogs (\textit{Echinops} and \textit{Setifer} tenrecs) % NC: Fix the brackets here, this ref should be in the same brackets as Setifer, two sets of brackets next to each other looks odd
...
% NC: Actually I don't think you need this subheading and the whole thing can just be one paragraph
