% NC: Note that in order for me to edit these properly I need your .bib file and all the figures. 
%If you use GitHub that makes it a lot easier as I can just clone the repo rather than you sending me millions of files!
...
\usepackage{graphicx}	% so we can add graphics
\usepackage{authblk}	% For author affiliations
...
\linespread{1.3} 		% this is 1.5 spacing, double line spacing is 1.6
...
Understanding patterns of biological diversity remains a central challenge in evolutionary biology. Investigations of why some groups are more diverse than others help us to elucidate the patterns and underlying mechanisms governing the evolutionary success of a clade (REF).% NC: Yes true, but how is this relevant to your question?

Many diverse groups such as Darwin's finches (REF), cichlid fish (REF), Caribbean \textit{Anolis} lizards (REF) and Hawaiian spiders (REF) are the products of adaptive radiations \citep{Losos2010}. Each of these groups are considered to be examples of exceptional diversity due to the variety of ecological niches which they occupy. % NC: Again, yes, but is this important?

The biological diversity of adaptively radiated groups can be characterised in two ways; species richness and phenotypic diversity. These metrics %NC: wouldn't call them metrics - that implies a proper measurement% 
...
% NC: OK I see what you're getting at here. But it's far to long before you get on to what you're actually looking at. How about a severely shortened version, something along the lines of 1 - Adaptive radiation, short definition, is an exciting pattern and seen across many taxa (some examples, probably mention Darwin's finches). There has been debate about the exact meaning of the term - some arguing that it should be reserved only for species exhibiting lots of morphological disparity. However, few adaptive radiations have been characterised in this way (if this is true :). 2 - Morphological disparity is short definition. It can be measured in many ways including - several *short* methods. 3- Here we investigate morphological disparity in tenrecs, to determine whether they represent an adaptive radiation sensu Losos. Tenrecs are - brief description. We use brief methods 4- *Brief$* results and implications. (It may take more than 4 paragraphs but I reckon this is a good starting framework. Remember it's just what the reader *needs* to know, not everything you know. I advise starting from scratch - I made Thomas do this several times too - it's *far* easier than trying to cut and paste what you already have)
...
Here we test whether there is evidence for significant morphological disparity in an adaptive radiation of small insectivore % NC: they aren't insectivore mammals, they're insectivorous - insectivore implies old taxonomic groupings%
mammals.

Tenrecs (Afrosoricida, Tenrecidae) are comprised of 34 species, 31 of which are endemic to Madagascar \citep{Olson2013}. The extant Malagasy tenrecs evolved from a single common ancestor \citep{Asher2006} which is thought to have colonised the island between 25 and 42 million years ago \citep{Samonds2013} % NC: relevance?
...
% NC: Predictions are fairly obvious so I don't think they need bullet pointing. Just integrate with text in methods paragraph
...
Furthermore, when compared to golden moles, tenrecs have a higher level of disparity in the shape of their skulls but not in their mandibles, indicating that tenrecs are more phenotypically diverse in some but not all aspects of their morphologies.% NC: The results should be shorter in the intro. Don't need to give *all* results, just main one(s).
...
\subsection{Morphological data collection} % NC: Perhaps more descriptive than geo morph?
...
One of us (SF) photographed cranial specimens of tenrecs and golden moles at the Natural History Museum London (NHML % Might want to use the standard museum abbreviations - BMNH etc.)
...
% NC: Photos are fine. If you want to remove backgrounds and stuff have a play with Inkscape (free to download)

% NC: Remember for a manuscript these will all be at the end
% NC: You probably want to do this like the template thesis I sent round and have the tables as separate files to
% NC: stick into the document. Will be easier to edit and you'll have less stuff cluttering the manuscript file. Also will make it easier for the thesis version.
...
% NC: I would generally just state the taxonomy I used and not explain anything. 
...
Finally, because our data contains more tenrecs than golden moles, and disparity is expected to be higher in larger groups (REFS), we repeated our group comparisons of disparity % NC: which analysis is this referring to???
...
\subsection{Morphological disparity in tenrecs} % NC: weird to give the answer in the subheading
...
% NC: Is there a figure of the distribution with your values on it? Would be nice. and possibly better than a table to show the results quickly and clearly.
...
\subsection{Morphological disparity in tenrec and golden moles} % NC: weird to give the answer in the subheading
...
\section{Discussion} % NC: I know this is rough and hard to do before the final results are in, so I haven't done too much to it. It looks a bit long already though. Same kind of rules apply as in the introduction. Should include 1 - summary of results 2 - interpret results with reference to other work 3 - Caveats 4 - Implications. Discussion is mainly meant to set your results in the context of other work. You don't need to include every little possible problem in the caveats.

Our findings provide new insights into phenotypic diversity within the tenrec family. Contrary to previous claims (REFS), tenrecs do not appear to be exceptional in their morphological diversity. They do seem to be more phenotypically diverse than their closest relatives but only in ventral skull morphology; the opposite is true when we look at mandible morphology (figure \ref{mandsPCA}). Our results illustrate the vital importance of applying quantitative methods to test assumptions about phenotypic evolution. % NC: Do you want to tie this back to adaptive radiation here? There are issues with semantics here. You use disparity and phenotypic diversity interchangeably. Be careful! What did you actually test? Stick to that term throughout.

\subsection{Disagreement among disparity metrics calculated for skulls and mandibles}
% NC: Do you need subheadings really?

Our three analyses of skull shape (dorsal, ventral and lateral views) all indicated that morphological disparity is greater in tenrecs than in golden moles. Intriguingly %NC: is it?
, we found the opposite pattern to be true for mandible shape.

There are no intuitive reasons why variation in phenotypic shape is decoupled between skulls and mandibles. One would assume that variation in mandible and skull shape should be strictly correlated. In particular, we expected patterns of diversity in the mandibles to reflect shape variation in the lateral views of the skulls (see supplementary). % NC: Really need to come up with some vague biological explanation before saying it's all errors in my data collection
...
% NC: Yeah possibly. It's ok to mention limitations, but try not to open yourself to too much criticism
...
% I need a paragraph here about why higher diversity in golden mole mandibles is possibly significant/interesting - I don't know yet!

\subsection{Is phenotypic diversity in tenrecs the product of an adaptive radiation?} %Again, is this needed? Don't use subheadings as an excuse not to link ideas and paragraphs together.

Tenrecs are evidently a diverse group, both phenotypically and ecologically. Body sizes of extant tenrecs span three orders of magnitude (2.5 to \textgreater 2,000g) which is a greater range than all other Families, and most Orders %NC: I think you use capital letters in the context of taxonomic groupings
...
However, our results cast doubt over whether the evident diversity within the tenrec family should be considered to be the product of an adaptive radiation % NC: is it the product of adaptive radiation OR adaptive radiation itself?
